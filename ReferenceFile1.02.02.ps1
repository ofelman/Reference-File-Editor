...
$Script:OS11Vers = @('21H2','22H2','23H2')
###################################################################################
$Script:OS
$Script:OSVer
...
Function Create_Form {
    ...
    $cf_CategoryLabel.Text = "Filter"
    $cf_CategoryLabel.location = New-Object System.Drawing.Point(($LeftOffset-10),($TopOffset-10))  # (from left, from top)
    $cf_CategoryLabel.Size = New-Object System.Drawing.Size(40,15)                      # (width, height)
    $cf_CategoryLabel.TextAlign = "BottomLeft"
    # Add Filter action script
    $cf_FilterAction = {        
        $cf_CategoriesChecked = @()
        $cf_DriverTypeChecked = @()
        if ( $cf_CategoryBIOS.checked ) { $cf_CategoriesChecked += 'BIOS'}
        if ( $cf_CategorySoftware.checked ) { $cf_CategoriesChecked += 'Software'}
        if ( $cf_CategoryFirmware.checked ) { $cf_CategoriesChecked += 'Firmware'}
        if ( $cf_CategoryDock.checked ) { $cf_CategoriesChecked += 'Dock'} 
        if ( $cf_CategoryDriver.checked ) { $cf_CategoriesChecked += 'Driver' ;
            if ( $cf_CategoriesChecked.count -eq 1 ) {
                $cf_TypeNetwork.Enabled = $true
                $cf_TypeGraphics.Enabled = $true
                $cf_TypeStorage.Enabled = $true
                $cf_TypeAudio.Enabled = $true
                $cf_TypeChipset.Enabled = $true
                if ( $cf_TypeNetwork.checked ) { $cf_DriverTypeChecked += 'Network' }
                if ( $cf_TypeGraphics.checked ) { $cf_DriverTypeChecked += 'Graphics' }
                if ( $cf_TypeStorage.checked ) { $cf_DriverTypeChecked += 'Storage' }
                if ( $cf_TypeAudio.checked ) { $cf_DriverTypeChecked += 'Audio' }
                if ( $cf_TypeChipset.checked ) { $cf_DriverTypeChecked += 'Chipset' }
            } else {
                $cf_TypeNetwork.Checked = $false        ; $cf_TypeNetwork.Enabled = $false
                $cf_TypeGraphics.Checked = $false       ; $cf_TypeGraphics.Enabled = $false
                $cf_TypeStorage.Checked = $false        ; $cf_TypeStorage.Enabled = $false
            ...
            $cf_TypeAudio.checked = $false ; $cf_TypeAudio.Enabled = $false
            $cf_TypeChipset.checked = $false ; $cf_TypeChipset.Enabled = $false
            #$cf_TypeOther.checked = $false ; $cf_TypeOther.Enabled = $false
        } # else if ( $cf_CategoryDriver.checked )

        $cf_XMLSolutions = $Script:xmlContent.SelectNodes("ImagePal/Solutions/UpdateInfo")
        $cf_SpqCount = Show_SoftpaqList $cf_SoftpaqsList $cf_XMLSolutions $cf_CategoriesChecked $cf_SolutionSearch.Text $cf_DriverTypeChecked
        $cf_CurrentSpqsMsg.Text = "$($cf_SpqCount) Softpaqs" # mark the action as 'done'
    ...
    $cf_CategoryBIOS.Location = New-Object System.Drawing.Point(($LeftOffset+40),($TopOffset-10))   # (from left, from top)
    $cf_CategoryDriver.Text = 'Driver' ;        $cf_CategoryDriver.Autosize = $true
    $cf_CategoryDriver.Location = New-Object System.Drawing.Point(($LeftOffset+90),($TopOffset-10))   # (from left, from top)
    $cf_CategorySoftware.Text = 'Software' ;    $cf_CategorySoftware.Autosize = $true
    $cf_CategorySoftware.Location = New-Object System.Drawing.Point(($LeftOffset+150),($TopOffset-10))   # (from left, from top)
    $cf_CategoryFirmware.Text = 'Firmware' ;    $cf_CategoryFirmware.Autosize = $true
    $cf_CategoryFirmware.Location = New-Object System.Drawing.Point(($LeftOffset+220),($TopOffset-10))   # (from left, from top)
    $cf_CategoryDock.Text = 'Dock' ;            $cf_CategoryDock.Autosize = $true
    $cf_CategoryDock.Location = New-Object System.Drawing.Point(($LeftOffset+290),($TopOffset-10))   # (from left, from top)

    $cf_CategoryBIOS.add_click($cf_FilterAction)
    $cf_CategoryDriver.add_click($cf_FilterAction)
    $cf_CategorySoftware.add_click($cf_FilterAction)
    $cf_CategoryFirmware.add_click($cf_FilterAction)
    $cf_CategoryDock.add_click($cf_FilterAction)

    # Add 'Driver Type' Filter label
    $cf_DriverFilterMsg = New-Object System.Windows.Forms.Label
    $cf_DriverFilterMsg.location = New-Object System.Drawing.Point(($LeftOffset-10),($TopOffset+20))  # (from left, from top)
    $cf_DriverFilterMsg.Size = New-Object System.Drawing.Size(80,20) # (width, height)
    $cf_DriverFilterMsg.Text = 'Driver Type'
    
    # Add 'Driver Type' checkboxe selections and actions
    $cf_TypeNetwork = New-Object System.Windows.Forms.CheckBox ;      $cf_TypeNetwork.Enabled = $false
    $cf_TypeGraphics = New-Object System.Windows.Forms.CheckBox ;     $cf_TypeGraphics.Enabled = $false
    $cf_TypeStorage = New-Object System.Windows.Forms.CheckBox ;      $cf_TypeStorage.Enabled = $false
    $cf_TypeAudio = New-Object System.Windows.Forms.CheckBox ;        $cf_TypeAudio.Enabled = $false
    $cf_TypeChipset = New-Object System.Windows.Forms.CheckBox ;      $cf_TypeChipset.Enabled = $false

    $cf_TypeNetwork.Text = 'Network' ;          $cf_TypeNetwork.Autosize = $true
    $cf_TypeNetwork.Location = New-Object System.Drawing.Point(($LeftOffset),($TopOffset+40))   # (from left, from top)   
    $cf_TypeGraphics.Text = 'Graphics' ;        $cf_TypeGraphics.Autosize = $true
    $cf_TypeGraphics.Location = New-Object System.Drawing.Point(($LeftOffset),($TopOffset+65))   # (from left, from top)   
    $cf_TypeStorage.Text = 'Storage' ;          $cf_TypeStorage.Autosize = $true
    $cf_TypeStorage.Location = New-Object System.Drawing.Point(($LeftOffset),($TopOffset+90))   # (from left, from top)
    $cf_TypeAudio.Text = 'Audio' ;              $cf_TypeAudio.Autosize = $true
    $cf_TypeAudio.Location = New-Object System.Drawing.Point(($LeftOffset),($TopOffset+115))   # (from left, from top)
    $cf_TypeChipset.Text = 'Chipset' ;          $cf_TypeChipset.Autosize = $true
    $cf_TypeChipset.Location = New-Object System.Drawing.Point(($LeftOffset),($TopOffset+140))   # (from left, from top)

    $cf_TypeGraphics.add_click($cf_FilterAction)
    $cf_TypeNetwork.add_click($cf_FilterAction)
    $cf_TypeStorage.add_click($cf_FilterAction)
    $cf_TypeAudio.add_click($cf_FilterAction)
    $cf_TypeChipset.add_click($cf_FilterAction)
<##>
    ...
    $cf_RefreshCatButton.location = New-Object System.Drawing.Point($LeftOffset,($TopOffset+165))  # (from left, from top)    
    ...
    $cf_RefreshCatButton.add_click( {
        ...
        $cf_SoftpaqField.Text = ""          # clear the to be replaced entry field
        $cf_SoftpaqReplace.Text = ""        # clear the replace entry field
        $cf_CurrentSpqsMsg.Text = "Done ($($cf_SpqCount))"    # mark the action as 'done'
        $cf_ssList.Items.Clear()            # clear out the solutions supersede listbox
    } ) # $cf_RefreshCatButton.add_click
    ...
    $SolutionGroupBox.Controls.AddRange(@($cf_DriverFilterMsg,$cf_TypeNetwork,$cf_TypeGraphics,$cf_TypeStorage,$cf_TypeAudio,$cf_TypeChipset))
    #$SolutionGroupBox.Controls.AddRange(@($cf_CurrentSpqsMsg,$cf_RefreshCatButton))
    $SolutionGroupBox.Controls.AddRange(@($cf_CurrentSpqsMsg))
    ...
    $cf_CurrentSpqsLabel.Text = "Current List"
    $cf_CurrentSpqsLabel.location = New-Object System.Drawing.Point(($LeftOffset+90),($TopOffset+20))  # (from left, from top)
    ...
    $cf_SolutionSearch.location = New-Object System.Drawing.Point(($LeftOffset+190),($TopOffset+20))  # (from left, from top)
    $cf_SolutionSearch.Size = New-Object System.Drawing.Size(110,20)  # (width, height)
    ...
    $cf_SolutionSearch.Add_KeyDown({
        if ($_.KeyCode -eq "Enter") { $cf_SolutionGo.PerformClick() }
    })
    $cf_SolutionGo = New-Object System.Windows.Forms.Button
    $cf_SolutionGo.Text = "Find"
    $cf_SolutionGo.location = New-Object System.Drawing.Point(($LeftOffset+300),($TopOffset+20))  # (from left, from top)
    $cf_SolutionGo.Size = New-Object System.Drawing.Size(50,20)  # (width, height)
    $cf_SolutionGo.Enabled = $false
    $cf_SolutionGo.add_click($cf_FilterAction)
    ...
    $cf_SoftpaqsList.location = New-Object System.Drawing.Point(($LeftOffset+90),60)  # (from left, from top)
    $cf_SoftpaqsList.Size = New-Object System.Drawing.Size(260,($FormHeight-360)) # (width, height)
    $cf_SoftpaqsList.HorizontalScrollBar="Auto"
    $cf_SoftpaqsList.SelectionMode = 'MultiExtended'
    # Events - https://learn.microsoft.com/en-us/dotnet/api/system.windows.forms.listbox?view=windowsdesktop-8.0
    $cf_SoftpaqsList.add_SelectedIndexChanged( {
        $cf_actionMsg.Text = ""
        if ( $cf_SoftpaqsList.SelectedItems.count -eq 1 ) {
            $cf_SoftpaqField.Text = $cf_SoftpaqsList.SelectedItem
            $ss_SoftpaqID = $cf_SoftpaqsList.SelectedItem.split(' ')[0]
            $cf_SpqCategory.Text = Get_SoftpaqCategory $ss_SoftpaqID
            List_Supersedes $cf_ssList $ss_SoftpaqID
    ...
    $cf_ssLabel.Text = "Superseded Selection List"
    $cf_ssLabel.location = New-Object System.Drawing.Point(($LeftOffset+360),($TopOffset+20))  # (from left, from top)
    ...
    $cf_ssList.location = New-Object System.Drawing.Point(($LeftOffset+360),($TopOffset+40))  # (from left, from top)
    ...
    $ReplaceGroupBox.location = New-Object System.Drawing.Point(($LeftOffset+360),($TopOffset+160)) # (from left, from top)
    ...
    # Add 'Category' Label
    $cf_SpqCategoryLabel = New-Object System.Windows.Forms.Label
    $cf_SpqCategoryLabel.Text = "Softpaq Category"
    $cf_SpqCategoryLabel.location = New-Object System.Drawing.Point(($LeftOffset+360),($TopOffset-10))
    $cf_SpqCategoryLabel.AutoSize = $true
    $cf_SpqCategoryLabel.TextAlign = "BottomCenter"
    # Add Softpaq Categoryfield
    $cf_SpqCategory = New-Object System.Windows.Forms.TextBox
    $cf_SpqCategory.location = New-Object System.Drawing.Point(($LeftOffset+460),($TopOffset-10))  # (from left, from top)
    $cf_SpqCategory.Size = New-Object System.Drawing.Size(140,20)  # (width, height)
    $cf_SpqCategory.BorderStyle = 'FixedSingle'                # Options: Fixed3D, FixedSingle, None (default)
    $cf_SpqCategory.ReadOnly = $true
    ...
    $SolutionGroupBox.Controls.AddRange(@($ReplaceGroupBox,$cf_SpqCategoryLabel,$cf_SpqCategory))
    #$cf_Form.Controls.Add($ReplaceGroupBox)
    
    $cf_Debug = New-Object System.Windows.Forms.CheckBox ;  $cf_Debug.Enabled = $true
    $cf_Debug.Text = 'Activity Log'
    ...
    $cf_RepoGroupBox.Text = 'Manage Custom HPIA Offline Repository'
    ...
    $cf_CurrentSpqsMsg.Text = 'Select Platform'
